...
public class SessionService {
    ...
    public Session updateSessionStatus(String id, SessionStatus status) {
        Session session = getSessionById(id);
    ...
    public Session completeSession(String id, Integer actualDurationMinutes, String notes) {
        ...
        // Record learning activity in streak system
        streakService.recordLearningActivity(
                session.getUserId(), 
    ...
    public Map<String, Object> getSessionAnalytics(String userId) {
        ...
        List<Session> completedSessions = allSessions.stream()
                .filter(s -> SessionStatus.COMPLETED.equals(s.getStatus()))
        ...
        // Get user streak info from streak service
        var userStreak = streakService.getUserStreak(userId);
        ...
        analytics.put("currentStreak", userStreak.getCurrentStreak());
        analytics.put("longestStreak", userStreak.getLongestStreak());
        analytics.put("totalLearningDays", userStreak.getTotalLearningDays());
        analytics.put("weeklyStudyMinutes", userStreak.getWeeklyStudyMinutes());
        analytics.put("totalNodesCompleted", userStreak.getTotalNodesCompleted());
        analytics.put("totalRoadmapsCompleted", userStreak.getTotalRoadmapsCompleted());
