...
export function ScheduleStats() {
  ...
  const upcomingSessions = sessions.filter(
    session => new Date(session.startTime) > new Date() && session.status === 'PLANNED'
  );
  ...
  const formatNextSession = () => {
    ...
    if (nextSession.toDateString() === today.toDateString()) {
      return "Today";
    } else if (nextSession.toDateString() === tomorrow.toDateString()) {
      return "Tomorrow";
    } else {
